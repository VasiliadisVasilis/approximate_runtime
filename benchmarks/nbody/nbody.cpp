...
#pragma acl task label("update") approxfun(update_Approx) device_out(pos, force, vel, potential, color) significant(0) workers(localwork[0],localwork[1]) groups(globalwork[0],globalwork[1])
  update(pos,color, force, vel, potential, bound, dt, particlesnumber);
#pragma acl taskwait label("update") ratio(0.0)
...
{	
  ...
  for (int i=0; i < particlesnumber; i++)
  {
    E_kin += 0.5 * ((double)vel[i].x*vel[i].x + (double)vel[i].y*vel[i].y + (double)vel[i].z*vel[i].z);
    E_V += potential[i];
  }
...
#pragma acl task label("update") approxfun(update_Approx) in(pos, force, vel, potential, color) significant(0) workers(localwork[0],localwork[1]) groups(globalwork[0],globalwork[1])
    update(pos,color, force, vel, potential, bound, dt, particlesnumber);
#pragma acl taskwait label("update") ratio(0.0)
    steps_counter++;
...
#pragma acl taskwait label("update") ratio(0.0)
  ...
  printf ("Total app time is = %10g seconds\n",
